# for getting histograms of final trait values and/or fitness levels

ECplot=function(M,L,generations,lb,ub,formula,mutrule,Delta,hardb){
...
#if ((j==100) || (j==generations)){		print(population)}
#if ((j==100) || (j==generations)){		print(c("Average---min---max for generation",j))}
		fitnesstat[j,1]=mean(population[,L+1])
		fitnesstat[j,2]=min(population[,L+1])
		fitnesstat[j,3]=max(population[,L+1])

#if ((j==100) || (j==generations)){		print(fitnesstat[j,])}
...
# return final avg fitness
return(fitnesstat[generations,1])

}

ecplotter2=array(dim=0)

for (i in 1:200) {ecplotter2=c(ecplotter2, ECplot(10,1,100,-100,100,1,1,10,1))}
